...
def build_custom_tools(organization=None):
    """Return a list of StructuredTool for the org's active custom tools.
    The org is captured in each tool's closure so dispatch stays correct inside
    ToolNode worker threads."""
    from knowledge.models import CustomTool

    qs = CustomTool.objects.filter(is_active=True)
    if organization is not None:
        qs = qs.filter(organization=organization)
    tools = []
    for ct in qs:
        try:
            tools.append(_make_structured_tool(ct, organization))
        except Exception as exc:  # never let a bad tool break the agent
            logger.error('[CustomTool] skipping "%s": %s', ct.name, exc)
    return tools


def active_tools_signature(organization=None) -> str:
    """A cheap key that changes whenever the org's active tool set changes."""
    from knowledge.models import CustomTool

    qs = CustomTool.objects.filter(is_active=True)
    if organization is not None:
        qs = qs.filter(organization=organization)
    rows = qs.values_list('id', 'updated_at')
    return '|'.join(f'{i}:{u.timestamp()}' for i, u in rows) or 'none'


def _make_structured_tool(ct, organization=None) -> StructuredTool:
    fields = {}
    ...
    def _runner(**kwargs):
        return _dispatch(ct, kwargs, organization)
...
def _dispatch(ct, args: dict, organization=None) -> str:
    """Execute a custom tool by archetype, logging an audit/billing record.
    `organization` is captured in the tool closure (reliable inside threads)."""
    from knowledge.models import CustomTool, CustomToolRun

    organization = organization or getattr(ct, 'organization', None)
    ...
    try:
        if ct.archetype == CustomTool.ARCHETYPE_COLLECT:
            ...
            result = _do_tag_route(ct, args, conv_id, organization)
        elif ct.archetype == CustomTool.ARCHETYPE_CANNED:
            result = _do_canned(ct, args, organization)
        elif ct.archetype == CustomTool.ARCHETYPE_WEBHOOK:
            result, cost = _do_webhook(ct, args, organization)
        else:
...
def _do_tag_route(ct, args, conv_id, organization=None) -> str:
...
def _do_canned(ct, args, organization=None) -> str:
    ...
    if doc_id:
        ...
        qs = KnowledgeDoc.objects.filter(pk=doc_id, is_active=True)
        if organization is not None:
            qs = qs.filter(organization=organization)
        doc = qs.first()
        if doc:
...
def _do_webhook(ct, args, organization=None):
    ...
    ws = Workspace.get_for_org(organization) if organization is not None else Workspace.get_solo()
    ...
    cost = _charge_tool_run(ws.tool_run_cost_usd, ct.name, organization)
...
def _charge_tool_run(cost_usd, tool_name, organization=None) -> Decimal:
    ...
    try:
        ...
        with db_tx.atomic():
            CreditAccount.objects.get_or_create(organization=organization)
            account = CreditAccount.objects.select_for_update().get(organization=organization)
